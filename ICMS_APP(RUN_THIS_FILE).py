    ...
    if len(lec_ls) > 0 or len(tut_ls) > 0:
        for i in range(max(len(lec_ls), len(tut_ls))):
            ...
            if len(lec_ls) > 0:
                ...
                delta_hour = lec_hour - today_hour
                delta_min = lec_min - today_min

                # Main page When having a lecture
                if delta_hour == 0 and delta_min > 0 and current_day == lec_f[8]:
                    layout_lec_left = [
            ...
            if len(tut_ls) > 0:
                ...
                delta2_hour = tut_hour - today_hour
                delta2_min = tut_min - today_min

                # 3. Main Page When having a tutorial
                if delta2_hour == 0 and delta2_min > 0 and current_day == tut_f[8]:
                    layout_tut_left = [
                        [sg.Text("ICMS HOME\n\n", font=('Helvetica', 24), justification='left')],
